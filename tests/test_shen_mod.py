...
def test_quality_runtime_uses_live_ui_paths_and_seeded_dragon_variants() -> None:
    ...
    assert '"version=0.10.16;root=' in source
    assert '"version=0.10.16;from_size=' in source
    native_dll = (MOD / "lol_mod.dll").read_bytes()
    assert b"version=0.10.16;root=" in native_dll
    assert b"version=0.10.16;from_size=" in native_dll
