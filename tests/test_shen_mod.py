...
def test_lucian_replaces_native_archer_002_and_is_localized() -> None:
    ...
    assert mod_info["version"] == "0.10.15"
...
def test_quality_runtime_uses_live_ui_paths_and_seeded_dragon_variants() -> None:
    ...
    assert '"version=0.10.15;root=' in source
    assert '"version=0.10.15;from_size=' in source
    native_dll = (MOD / "lol_mod.dll").read_bytes()
    assert b"version=0.10.15;root=" in native_dll
    assert b"version=0.10.15;from_size=" in native_dll
