...
FRAME_MANIFEST = MOD_ROOT / "source/native/yone_v5/frames.json"
...
EXPECTED_ROUTE = "exact-native-v5"
...
GOLDEN_IDLE_KEY = ("idle", 0)
GOLDEN_FACE_MIN_SIZE = (8, 9)
GOLDEN_CONNECTED_SKIN_MIN = 24
GOLDEN_CARD_SKIN_MIN = 96

# V3 and V4 actor/body sources are deliberately retired by the V5 route.  The
...
RETIRED_BODY_PREFIXES = ("source/native/yone_v4/",)
...
def _fail(message: str) -> None:
    raise V5ValidationError(message)


def _read_json(path: Path) -> Any:
    if not path.is_file():
        _fail(f"missing required V5 file: {path}")
    try:
...
def _safe_relative_file(base: Path, raw: object, *, label: str) -> Path:
    ...
    if relative.is_absolute() or any(part in {"", ".", ".."} for part in relative.parts):
        _fail(f"{label} escapes the V5 source root: {raw!r}")
...
def _rgba_pixels(image: Image.Image) -> list[tuple[int, int, int, int]]:
    if image.mode != "RGBA":
        _fail(f"V5 source must be RGBA, got {image.mode}")
...
def load_palette(path: Path) -> Palette:
    ...
    if payload.get("schema_version") != 5 or payload.get("route") != EXPECTED_ROUTE:
        _fail("palette.json must declare schema_version=5 and exact-native-v5")
...
def validate_frame_annotations(
    ...
    *,
    ...
    """Validate explicit V5 coordinates and the readable front-idle face."""
    ...
    golden_idle = (action, index) == GOLDEN_IDLE_KEY
    if golden_idle and face_visibility is not None and face_visibility != "front":
        _fail(f"{label} golden idle must declare face_visibility='front'")
    ...
    if face_required and (face_raw is None or mask_raw is None or not eyes_raw):
        _fail(f"{label} is a front-facing frame and needs explicit face/eye/mask annotations")
    if face_raw is not None and (mask_raw is None or not eyes_raw):
        _fail(f"{label} declares a visible face but lacks explicit eye/mask annotations")
    if face_raw is None:
        if eyes_raw:
        ...
        minimum_size = GOLDEN_FACE_MIN_SIZE if golden_idle else (6, 7)
        ...
        minimum_skin = GOLDEN_CONNECTED_SKIN_MIN if golden_idle else (12 if face_required else 4)
        ...
        if golden_idle and connected_skin < GOLDEN_CONNECTED_SKIN_MIN:
            _fail(
                f"{label} largest real connected skin block is {connected_skin}px; "
                f"minimum is {GOLDEN_CONNECTED_SKIN_MIN}px"
            )
        ...
        eye_colors = palette.semantic_colors("eye")
        eye_outline_colors = palette.exact_role("eye_outline")
        ...
        for eye_index, point in enumerate(eyes_raw):
            ...
            if image.getpixel((x, y)) not in eye_colors:
                _fail(
                    f"{label}.eye_pixels[{eye_index}] must point to exact eye_outline or "
                    "eye_highlight role; skin/mask role names never count"
                )
        ...
        if face_required and not outline_points:
            _fail(f"{label} needs at least one dark eye_outline pixel inside face_bbox")
        if golden_idle:
            framed_eye = any(
            ...
            # eye is sufficient; V5 deliberately does not demand two dots.
...
def _validate_card_preview(
    ...
    if preview.format != "PNG" or preview.mode != "RGBA" or preview.size != CARD_PREVIEW_SIZE:
        _fail(
            f"V5 body_preview must be RGBA {CARD_PREVIEW_SIZE}, got "
    ...
    if set(preview.getchannel("A").getdata()) != {255}:
        _fail("V5 body_preview must be the complete opaque 141x138 card surface")
    ...
    if preview.tobytes() != expected.tobytes():
        _fail(
            "V5 body_preview is not the exact complete 141x138 card route "
    ...
    if actor_bbox is None:
        _fail("V5 card actor route is empty")

    divider_clearance = CARD_DIVIDER_Y - actor_bbox[3]
    if divider_clearance < CARD_DIVIDER_CLEARANCE:
        _fail(
            f"V5 actor feet/weapon approach the card divider: clearance={divider_clearance}px"
        )
    if actor_mask.crop(CARD_UI_ICON_SAFE_RECT).getbbox() is not None:
        _fail(f"V5 actor overlaps the card UI icon safe rect {CARD_UI_ICON_SAFE_RECT}")
    if actor_mask.crop((0, CARD_DIVIDER_Y, 141, 138)).getbbox() is not None:
        _fail("V5 actor enters the divider/name band")
    ...
    if rendered_skin < GOLDEN_CARD_SKIN_MIN:
        _fail(
            f"V5 real card actor has only {rendered_skin} scaled skin pixels; "
            f"minimum is {GOLDEN_CARD_SKIN_MIN}"
        )
    rendered_face = (
...
def _validate_retired_paths(mod_root: Path) -> None:
    ...
    if existing:
        _fail("retired V3/V4 body paths still exist:\n" + "\n".join(sorted(set(existing))))
    ...
    if leaked:
        _fail("retired V3/V4 body paths entered release manifest:\n" + "\n".join(leaked))
...
def validate_v5(
    ...
    *,
    ...
    if payload.get("schema_version") != 5 or payload.get("route") != EXPECTED_ROUTE:
        _fail("frames.json must declare schema_version=5 and exact-native-v5")
    ...
    if not atlas_path.is_file() or not anim_path.is_file():
        _fail("V5 runtime actor atlas/anim is missing")
    atlas = Image.open(atlas_path)
    if atlas.format != "PNG" or atlas.mode != "RGBA" or atlas.size != EXPECTED_ATLAS_SIZE:
        _fail(
            f"V5 actor atlas must be RGBA {EXPECTED_ATLAS_SIZE}, got "
    ...
    for position, row in enumerate(rows):
        ...
        if key in seen_keys:
            _fail(f"duplicate V5 frame key: {label}")
        seen_keys.add(key)
        if key not in expected:
            _fail(f"unexpected V5 body frame: {label}")
        ...
        bright_floor = 6 if action == "dead" and row.get("face_visibility") == "hidden" else 8
        ...
        used_opaque_colors.update(
            color for color in source.getdata() if color[3] == 255
        )
    ...
    if seen_keys != set(expected):
        _fail(f"frames.json does not cover exact 54-frame contract: missing={sorted(set(expected) - seen_keys)}")
    if any(visibility_presence) and not all(visibility_presence):
        _fail("face_visibility must be present on all 54 frames or omitted from all 54")
    if len(used_opaque_colors) > MAX_OPAQUE_PALETTE_COLORS:
        _fail(
            f"V5 body frames use {len(used_opaque_colors)} opaque colors; maximum is "
            f"{MAX_OPAQUE_PALETTE_COLORS}"
        )

    preview_report = _validate_card_preview(
        preview_path,
        sources[GOLDEN_IDLE_KEY],
        rows_by_key[GOLDEN_IDLE_KEY],
        palette,
    )
    ...
    return {
        ...
        "retired_v3_v4_body_paths_absent": True,
    }
...
def main() -> int:
    try:
        report = validate_v5()
    except V5ValidationError as error:
        print(f"Yone V5 exact-native validation FAILED: {error}", file=sys.stderr)
        return 1
    print(
        "Yone V5 exact-native validation passed: "
