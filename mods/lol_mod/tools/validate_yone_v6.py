...
FRAME_MANIFEST = MOD_ROOT / "source/native/yone_v6/frames.json"
GENERATION_QA = MOD_ROOT / "source/native/yone_v6/generation_qa.json"
...
EXPECTED_ROUTE = "exact-native-v6"
...
SOURCE_PATHS = {
    "motion": MOD_ROOT / "source/imagegen/yone_v6_motion_contact.png",
    "attack_q_w": MOD_ROOT / "source/imagegen/yone_v6_attack_q_w_contact.png",
    "w": MOD_ROOT / "source/imagegen/yone_v6_w_contact.png",
    "ult": MOD_ROOT / "source/imagegen/yone_v6_ult_contact.png",
}

# V3, V4, and V5 actor/body sources are retired by the final V6 route.  The
...
RETIRED_BODY_PREFIXES = ("source/native/yone_v4/", "source/native/yone_v5/")
...
class V6ValidationError(ValueError):
    pass
...
def _fail(message: str) -> None:
    raise V6ValidationError(message)


def _read_json(path: Path) -> Any:
    if not path.is_file():
        _fail(f"missing required V6 file: {path}")
    try:
...
def _sha256(path: Path) -> str:
    ...
    try:
        ...
        _fail(f"cannot hash V6 source {path}: {error}")
...
def _validate_generation_qa(mod_root: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        _fail("V6 generation_qa.json must be an object")
    if payload.get("schema_version") != 6 or payload.get("route") != EXPECTED_ROUTE:
        _fail("generation_qa.json must declare schema_version=6 and exact-native-v6")
    ...
    if payload.get("source_hashes") != EXPECTED_SOURCE_HASHES:
        _fail("generation_qa.json source_hashes do not match the accepted V6 sources")
    if payload.get("source_to_native_resampling") != "LANCZOS":
        _fail("V6 source-to-native route must declare LANCZOS resampling")
    ...
    for label, canonical_path in SOURCE_PATHS.items():
        ...
        if not path.is_file():
            _fail(f"missing hash-locked V6 source: {path}")
        actual_hashes[label] = _sha256(path)
    if actual_hashes != EXPECTED_SOURCE_HASHES:
        _fail(f"hash-locked V6 source changed: {actual_hashes}")
    return {
        "schema_version": 6,
...
def _safe_relative_file(base: Path, raw: object, *, label: str) -> Path:
    ...
    if relative.is_absolute() or any(part in {"", ".", ".."} for part in relative.parts):
        _fail(f"{label} escapes the V6 source root: {raw!r}")
...
def _rgba_pixels(image: Image.Image) -> list[tuple[int, int, int, int]]:
    if image.mode != "RGBA":
        _fail(f"V6 source must be RGBA, got {image.mode}")
...
def load_palette(path: Path) -> Palette:
    ...
    if payload.get("schema_version") != 6 or payload.get("route") != EXPECTED_ROUTE:
        _fail("palette.json must declare schema_version=6 and exact-native-v6")
...
def _validate_v6_quality(image: Image.Image, *, action: str, index: int) -> dict[str, Any]:
    ...
    if contrast < 58:
        _fail(f"{label} V6 luminance contrast is too low: {contrast:.2f}")
    if not 0.06 <= dark_ratio <= 0.84:
        _fail(f"{label} V6 dark ratio is outside 0.06..0.84: {dark_ratio:.4f}")
    if density < 0.10:
        _fail(f"{label} V6 body density is too low: {density:.4f}")
...
def validate_frame_annotations(
    ...
    *,
    ...
    """Validate explicit V6 coordinates and the readable front-idle face."""
    ...
    if face_raw is None:
        ...
        if face_required:
            ...
            # eye is sufficient; V6 deliberately does not demand two dots.
...
def _validate_card_preview(
    ...
    if preview.format != "PNG" or preview.mode != "RGBA" or preview.size != CARD_PREVIEW_SIZE:
        _fail(
            f"V6 body_preview must be RGBA {CARD_PREVIEW_SIZE}, got "
    ...
    if set(_pixels(preview.getchannel("A"))) != {255}:
        _fail("V6 body_preview must be the complete opaque 141x138 card surface")
    ...
    if preview.tobytes() != expected.tobytes():
        _fail(
            "V6 body_preview is not the exact complete 141x138 card route "
    ...
    if actor_bbox is None:
        _fail("V6 card actor route is empty")

    divider_clearance = CARD_DIVIDER_Y - actor_bbox[3]
    if divider_clearance < CARD_DIVIDER_CLEARANCE:
        _fail(
            f"V6 actor feet/weapon approach the card divider: clearance={divider_clearance}px"
        )
    if actor_mask.crop(CARD_UI_ICON_SAFE_RECT).getbbox() is not None:
        _fail(f"V6 actor overlaps the card UI icon safe rect {CARD_UI_ICON_SAFE_RECT}")
    if actor_mask.crop((0, CARD_DIVIDER_Y, 141, 138)).getbbox() is not None:
        _fail("V6 actor enters the divider/name band")
    ...
    if rendered_skin < IDLE_CARD_SKIN_MIN:
        _fail(
            f"V6 real card actor has only {rendered_skin} scaled skin pixels; "
...
def _validate_retired_paths(mod_root: Path) -> None:
    ...
    if existing:
        _fail("retired V3/V4/V5 body paths still exist:\n" + "\n".join(sorted(set(existing))))
    ...
    if leaked:
        _fail("retired V3/V4/V5 body paths entered release manifest:\n" + "\n".join(leaked))
...
def validate_v6(
    ...
    *,
    ...
    if payload.get("schema_version") != 6 or payload.get("route") != EXPECTED_ROUTE:
        _fail("frames.json must declare schema_version=6 and exact-native-v6")
    ...
    if not anim_path.is_file():
        _fail("V6 runtime actor anim is missing")
    ...
    if verify_runtime_atlas:
        ...
        if not atlas_path.is_file():
            _fail("V6 runtime actor atlas is missing")
        atlas = Image.open(atlas_path)
        if atlas.format != "PNG" or atlas.mode != "RGBA" or atlas.size != EXPECTED_ATLAS_SIZE:
            _fail(
                f"V6 actor atlas must be RGBA {EXPECTED_ATLAS_SIZE}, got "
    ...
    for position, row in enumerate(rows):
        ...
        label = f"{action}[{index}]"
        if key in seen_keys:
            _fail(f"duplicate V6 frame key: {label}")
        seen_keys.add(key)
        if key not in expected:
            _fail(f"unexpected V6 body frame: {label}")
        ...
        quality_stats = _validate_v6_quality(source, action=action, index=index)
        reports[label] = {
            "file": file_raw,
    ...
    if len(used_opaque_colors) > MAX_OPAQUE_PALETTE_COLORS:
        _fail(
            f"V6 body frames use {len(used_opaque_colors)} opaque colors; maximum is "
            f"{MAX_OPAQUE_PALETTE_COLORS}"
        )
    ...
    return {
        ...
        "retired_v3_v4_v5_body_paths_absent": verify_retired_paths,
    }
...
def main() -> int:
    try:
        report = validate_v6()
    except V6ValidationError as error:
        print(f"Yone V6 exact-native validation FAILED: {error}", file=sys.stderr)
        return 1
    print(
        "Yone V6 exact-native validation passed: "
