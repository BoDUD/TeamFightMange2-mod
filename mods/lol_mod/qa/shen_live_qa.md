...
- [ ] Q first sends one transparent, no-damage `lol_shen_twilight_assault_blade_anchor` to establish the selected enemy-side endpoint. Its registered transparent view must render nothing; `lol_shen_twilight_assault_blade_recall` is the visible cyan spirit blade that travels from that endpoint back to Shen, flashes on arrival, then grants exactly three normal 20 + 20% AP empowered attacks.
...
Latest automated startup smoke: 2026-07-15 18:15 JST, after rebuilding and installing the Shen Q/E and context-isolated Yone E corrections. The game reached `asset loading done!` with zero Shen/Yone/`lol_mod` loader errors and zero panic/fatal lines. The remaining `network asset load error: UnexpectedEof` default-banpick fallback and Workshop item 3736031680 warning are unrelated to this mod.
