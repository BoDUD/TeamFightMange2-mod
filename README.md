...
- 以同 ID `cavalry_knight` 覆盖官方英雄 006（骑兵），保留原生动画与三主动技能界面契约；克烈保持骑乘斯嘎尔的可读模型，并加入 ImageGen 生成的角色、技能特效、图标与 BP 插画，以及可审计的官方克烈音效。
- 三个主动槽按 Q/E/R 展示：Q 槽合并 Q 飞索捕熊器与 E 比武冲锋的突进、拴链和拉回；E 槽映射 W 暴烈秉性的四次攻击状态机；R 槽实现正面冲锋、加速、护盾、控制免疫与碰撞命中。
- 当前为数据层首版限制：克烈始终处于骑乘状态，暂不实现下马与重新上马；拴链延迟结算不二次检查距离；R 沿选定直线冲锋，不实现 LoL 原版自动寻路或途中转向。
