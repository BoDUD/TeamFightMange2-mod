...
## v0.11.0：慎、厄加特与永恩联合复测包

- 合并 PR #9 的慎 Q/E、厄加特稳定性/E 背摔、五名英雄固定战斗尺寸与希维尔计分栏头像修复，以及 PR #10 的永恩 Q3 击飞与 E 灵体 + W 月牙组合重构。
- 这是本地联合测试构建，不会改变两个 Draft PR 的审核状态；GitHub 合并仍等待实机复测后再次确认。
...
## 永恩 Q 三段、E 灵体与 W 月牙重构

- 以 `dual_blader` 同 ID 覆盖官方英雄 009，严格保留三个主动槽：`skill=Q`、`skill2=E 灵体 + W 月牙组合近似`、`ult=R`；不会伪造第四个按钮、注册重复 `lol_yone` 或混入厄加特分支。
...
- 第二主动槽删除旧追背、背后位移和击飞：施法范围 48000 与 W 月牙长度一致，五帧 `skill2_attack` 同时用于真实技能动作与 QA；施法点留下固定本体标记，可见灵体投射物离体后通过 `BackToCasterLinearProjectile` 返回。W 使用两个同形状纯数据命中体：伤害体对英雄、小兵与野怪各结算一次伤害，英雄探针不含攻击且只让前两名英雄触发封顶护盾。公开数据API不能写入并恢复真实英雄坐标，因此永恩本体留在标记处，文案不宣称真实E位置回溯。
- 永恩主体、钢剑/亚扎卡纳普攻、Q/E/W/R 特效、图标、紧凑头像、90×122 BP 英雄格、百科全身图与 BP 插画均来自独立 ImageGen 源或获准主体源的确定性派生；严格保留官方 Dual Blader 的13个动画标签、帧矩形、帧数和时长，并为所有动作保留脚底安全区。
